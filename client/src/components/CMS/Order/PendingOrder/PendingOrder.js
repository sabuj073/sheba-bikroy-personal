...
const PendingOrder = () => {
    ...
    const getnewOrder = async () => {
        await instance.post("get-new-order").then((response) => {
          if (neworders.length == 0 && Number(response.data.length) > 0) {
            setneworders(response.data);
            setneworderscount(Number(response.data.length));
          }
        });
      };
      getnewOrder();
