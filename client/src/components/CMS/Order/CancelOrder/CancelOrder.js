...
const CancelOrder = () => {
    ...
    const [cancelorderscount, setcancelorderscount] = useState([]);
    const getcancelOrder = async () => {
        await instance.post("get-cancel-order").then((response) => {
          if (cancelorders.length == 0 && Number(response.data.length) > 0) {
            setcancelorders(response.data);
            setcancelorderscount(Number(response.data.length));
          }
        });
      };
      getcancelOrder();
                            ...
                            <tr>
                                ...
                                <td>
                                    <select  value={val.status} className={styles.orderSelect} name="" id="" style={{ background: '#e74c3c' }} >
                                        ...
                                        <option  style={{ background: '#3498db' }} value="cancel">cancel</option>
